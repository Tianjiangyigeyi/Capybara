    ...
    Shader::Shader(const std::string& vertexSrc, const std::string& fragmentSrc)
    {
       // Create an empty vertex shader handle
		...
		if(isCompiled == GL_FALSE)
		{
			...
			CPBR_CORE_ERROR("{0}", infoLog.data());
			CPBR_CORE_ASSERT(false, "Vertex shader compilation failed!");
			return;
		}
		...
		if (isCompiled == GL_FALSE)
		{
			...
			CPBR_CORE_ERROR("{0}", infoLog.data());
			CPBR_CORE_ASSERT(false, "Fragment shader compilation failed!");
			return;
		...
		if (isLinked == GL_FALSE)
		{
			...
			CPBR_CORE_ERROR("{0}", infoLog.data());
			CPBR_CORE_ASSERT(false, "Shader link failed!");
			return;
		}
