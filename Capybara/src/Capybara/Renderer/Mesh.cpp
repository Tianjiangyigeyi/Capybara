...
namespace Capybara {
	...
	struct LogStream : public Assimp::LogStream
	{
		...
		{
			CPBR_CORE_ERROR("Assimp error: {0}", message);
		}
	};
		...
		: m_FilePath(filename)
	{
		...
		for (size_t m = 0; m < scene->mNumMeshes; m++)
		{
			...
			CPBR_CORE_ASSERT(mesh->HasPositions(), "Meshes require positions.");
			CPBR_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
			...
			{
				CPBR_CORE_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices.");
		...
		if (scene->HasMaterials())
		{
			...
			for (uint32_t i = 0; i < scene->mNumMaterials; i++)
			{
				...
				{
					CPBR_CORE_TRACE("Mesh has no normal map");
				}
				// Roughness Maps
				...
				{
					CPBR_CORE_TRACE("Mesh has no roughness map");
				}
				// Metalness  Maps
				...
				{
					CPBR_CORE_TRACE("Mesh has no metalness map");
				}
				continue;
	...
	{
		CPBR_CORE_ASSERT(pNodeAnim->mNumRotationKeys > 0);
	...
	{
		CPBR_CORE_ASSERT(pNodeAnim->mNumScalingKeys > 0);
	...
	glm::vec3 Mesh::InterpolateTranslation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		CPBR_CORE_ASSERT(NextPositionIndex < nodeAnim->mNumPositionKeys);
		float DeltaTime = (float)(nodeAnim->mPositionKeys[NextPositionIndex].mTime - nodeAnim->mPositionKeys[PositionIndex].mTime);
		float Factor = (animationTime - (float)nodeAnim->mPositionKeys[PositionIndex].mTime) / DeltaTime;
		if (Factor < 0.0f)
			Factor = 0.0f;
		CPBR_CORE_ASSERT(Factor <= 1.0f, "Factor must be below 1.0f");
	...
	glm::quat Mesh::InterpolateRotation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		CPBR_CORE_ASSERT(NextRotationIndex < nodeAnim->mNumRotationKeys);
	...
	glm::vec3 Mesh::InterpolateScale(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		CPBR_CORE_ASSERT(nextIndex < nodeAnim->mNumScalingKeys);
	...
	void Mesh::DumpVertexBuffer()
	{
		...
		CPBR_CORE_TRACE("------------------------------------------------------");
		CPBR_CORE_TRACE("Vertex Buffer Dump");
		CPBR_CORE_TRACE("Mesh: {0}", m_FilePath);
		if (m_IsAnimated)
		{
			for (size_t i = 0; i < m_AnimatedVertices.size(); i++)
			{
				auto& vertex = m_AnimatedVertices[i];
				CPBR_CORE_TRACE("Vertex: {0}", i);
				CPBR_CORE_TRACE("Position: {0}, {1}, {2}", vertex.Position.x, vertex.Position.y, vertex.Position.z);
				CPBR_CORE_TRACE("Normal: {0}, {1}, {2}", vertex.Normal.x, vertex.Normal.y, vertex.Normal.z);
				CPBR_CORE_TRACE("Binormal: {0}, {1}, {2}", vertex.Binormal.x, vertex.Binormal.y, vertex.Binormal.z);
				CPBR_CORE_TRACE("Tangent: {0}, {1}, {2}", vertex.Tangent.x, vertex.Tangent.y, vertex.Tangent.z);
				CPBR_CORE_TRACE("TexCoord: {0}, {1}", vertex.Texcoord.x, vertex.Texcoord.y);
				CPBR_CORE_TRACE("--");
			}
		}
		else
