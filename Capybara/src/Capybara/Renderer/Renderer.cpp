...
namespace Capybara
{
    ...
    {
        CPBR_CORE_ASSERT(renderPass, "Render pass cannot be null!");
        ...
        if (clear)
        {
            const glm::vec4& clearColor = renderPass->GetSpecification().TargetFramebuffer->GetSpecification().
                                                      ClearColor;
            Renderer::Submit([=]()
            {
    ...
    {
        CPBR_CORE_ASSERT(s_Data.m_ActiveRenderPass,
