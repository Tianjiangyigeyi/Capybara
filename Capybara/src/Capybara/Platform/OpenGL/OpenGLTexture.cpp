...
namespace Capybara {
		...
		: m_Format(format), m_Width(width), m_Height(height), m_Wrap(wrap)
	{
		...
		CPBR_RENDER_1(self, {
			...
			glTexImage2D(GL_TEXTURE_2D, 0, CapybaraToOpenGLTextureFormat(self->m_Format), self->m_Width, self->m_Height, 0, CapybaraToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, nullptr);
		...
		: m_FilePath(path)
	{
		...
		CPBR_RENDER_S1(srgb, {
			...
			if (srgb)
			{
				...
				CPBR_CORE_INFO("Creating srgb texture width {0} mips", levels);
			...
			else
			{
				...
				glTexImage2D(GL_TEXTURE_2D, 0, CapybaraToOpenGLTextureFormat(self->m_Format), self->m_Width, self->m_Height, 0, srgb ? GL_SRGB8 : CapybaraToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, self->m_ImageData.Data);
				glGenerateMipmap(GL_TEXTURE_2D);
	...
	void OpenGLTexture2D::Unlock()
	{
		m_Locked = false;
		CPBR_RENDER_S({
			glTextureSubImage2D(self->m_RendererID, 0, 0, 0, self->m_Width, self->m_Height, CapybaraToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, self->m_ImageData.Data);
		});
	}
		...
		: m_FilePath(path)
	{
		...
		CPBR_RENDER_S3(faces, faceWidth, faceHeight, {
			glGenTextures(1, &self->m_RendererID);
			...
			auto format = CapybaraToOpenGLTextureFormat(self->m_Format);
