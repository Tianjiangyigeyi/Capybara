...
namespace Capybara {
	...
	static GLenum CapybaraToOpenGLTextureFormat(TextureFormat format)
	{
		switch (format)
		{
			case Capybara::TextureFormat::RGB:     return GL_RGB;
			case Capybara::TextureFormat::RGBA:    return GL_RGBA;
			case Capybara::TextureFormat::Float16: return GL_RGBA16F;
		}
		CPBR_CORE_ASSERT(false, "Unknown texture format!");
		return 0;
	}
		...
		: m_Format(format), m_Width(width), m_Height(height), m_Wrap(wrap)
	{
		...
		Renderer::Submit([this]()
		{
			...
			glTexImage2D(GL_TEXTURE_2D, 0, CapybaraToOpenGLTextureFormat(m_Format), m_Width, m_Height, 0, CapybaraToOpenGLTextureFormat(m_Format), GL_UNSIGNED_BYTE, nullptr);
		...
		: m_FilePath(path)
	{
		...
		{
			CPBR_CORE_INFO("Loading HDR texture {0}, srgb={1}", path, srgb);
		...
		{
			CPBR_CORE_INFO("Loading texture {0}, srgb={1}", path, srgb);
			m_ImageData.Data = stbi_load(path.c_str(), &width, &height, &channels, srgb ? STBI_rgb : STBI_rgb_alpha);
			CPBR_CORE_ASSERT(m_ImageData.Data, "Could not read image!");
		...
		Renderer::Submit([=]()
		{
			...
			else
			{
				...
				GLenum internalFormat = CapybaraToOpenGLTextureFormat(m_Format);
				GLenum format = srgb ? GL_SRGB8 : (m_IsHDR ? GL_RGB : CapybaraToOpenGLTextureFormat(m_Format)); // HDR = GL_RGB for now
	...
	void OpenGLTexture2D::Unlock()
	{
		m_Locked = false;
		Renderer::Submit([this]() {
			glTextureSubImage2D(m_RendererID, 0, 0, 0, m_Width, m_Height, CapybaraToOpenGLTextureFormat(m_Format), GL_UNSIGNED_BYTE, m_ImageData.Data);
		});
	}

	void OpenGLTexture2D::Resize(uint32_t width, uint32_t height)
	{
		CPBR_CORE_ASSERT(m_Locked, "Texture must be locked!");
	...
	OpenGLTextureCube::OpenGLTextureCube(TextureFormat format, uint32_t width, uint32_t height)
	{
		...
		Renderer::Submit([=]() {
			...
			glTextureStorage2D(m_RendererID, levels, CapybaraToOpenGLTextureFormat(m_Format), width, height);
			glTextureParameteri(m_RendererID, GL_TEXTURE_MIN_FILTER, levels > 1 ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
			glTextureParameteri(m_RendererID, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		...
		: m_FilePath(path)
	{
		...
		CPBR_CORE_ASSERT(faceWidth == faceHeight, "Non-square faces!");
		...
		Renderer::Submit([=]() {
			...
			auto format = CapybaraToOpenGLTextureFormat(m_Format);
